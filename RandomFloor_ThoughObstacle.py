"""
this file is to simulate the behavior of water and solid obstacles,as waterramp dataset in learning to simulate
but it has some problems that the fluid particle will throw the obstacle,
...
ti.init(arch=ti.cpu) # Try to run on CPU/GPU
quality = 1 # Use a larger value for higher-res simulations
n_particles, n_grid = 1000 * quality ** 2, 128 * quality
dx, inv_dx = 1 / n_grid, float(n_grid)
dt = 1e-4 / quality
p_vol, p_rho = (dx * 0.5)**2, 1
p_mass = p_vol * p_rho
E, nu = 400, 0.2 # Young's modulus and Poisson's ratio
...
gravity = 9.8
r_v = 0   # velocity of the rigid obstacle movement
...
x_ls = [0.95, 0.6]  # left side of rigid obstacles
x_le = [0.3, 0.4]  # right side of rigid obstacles
...
def initialize():
    for i in range(n_particles):
        x[i] = [ti.random() * 0.4 + 0.3, ti.random() * 0.3 + 0.6]  # position of fluid
        v[i] = ti.Matrix([0, 0])
...
# obstacles shape
    ...
    for i in range(n_rseg):
        x_r[i + 1] = [x_ls[0] + (x_le[0] - x_ls[0]) / n_rseg * (i + 1),
                      x_ls[1] + (x_le[1] - x_ls[1]) / n_rseg * (i + 1)]
        x_rp[i] = (x_r[i] + x_r[i + 1]) / 2


initialize()
gui = ti.GUI("waterRamp", res=512, background_color=0x112F41)
...
while not gui.get_event(ti.GUI.ESCAPE, ti.GUI.EXIT):
    ...
    gui.line(x_r.to_numpy()[0], x_r.to_numpy()[-1], radius=3, color=0xFF0000)
    ...
    for i in range(p_A_.shape[0]):
        if p_T_[i] == 1:
            gui.circle(x_[i], radius=0.5, color=0xCD00CD)
        if p_T_[i] == -1:
            gui.circle(x_[i], radius=0.5, color=0x436EEE)

    # filename = f'pic_cutSolid/frame_{frame:05d}.png'
    # gui.show(filename)
    gui.show()
    frame += 1
